...
def parse_args():
    ...
    if args.model_config is None:
        ...
        args.model_config = str(
            Path(args.model_path) / "mask2former_swin-s-p4-w7-224_8xb2-lsj-50e_coco.py"
        )
    return args
...
def load_models(args):
    ...
    CKPT_PATH = str(
        list(Path(args.model_path).glob("mask2former_swin-s-p4-w7-224_8xb2-lsj-50e_coco*.pth"))[0]
    )
...
def relative_position(obj_a, obj_b):
    ...
    #
    revised_offset = np.maximum(np.abs(offset) - POSITION_THRESHOLD * (dim_a + dim_b), 0) * np.sign(
        offset
    )
...
def evaluate_image(filepath, metadata):
    ...
    if hasattr(result, "pred_instances"):
        ...
        masks = (
            pred_instances.masks.cpu().numpy()
        ...
        if len(scores) > 0:
            ...
            for idx in top_indices:
                cls_name = (
                    classnames[int(labels[idx])]
                    if int(labels[idx]) < len(classnames)
                    else f"cls{int(labels[idx])}"
                )
    ...
    return {
        ...
        "reason": reason,
        "metadata": metadata,
        "details": {key: [box.tolist() for box, _ in value] for key, value in detected.items()},
    }
